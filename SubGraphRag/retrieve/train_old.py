...
def main(args):
    ...
    # Try to initialize wandb, but proceed if it fails
    global use_wandb, log_dir
    try:
        wandb.init(
        ...
        print(f"Warning: Could not initialize wandb ({str(e)}). Will log to local files only.")
        use_wandb = False
        
        # Create a log directory and save config for local logging
    ...
    for epoch in range(config['train']['num_epochs']):
        ...
        log_metrics(train_log_dict, step=epoch)
        if num_patient_epochs == config['train']['patience']:
            break

def log_metrics(metrics, step=None):
    """Helper function to log metrics to both wandb and local file if wandb is not available"""
    if use_wandb:
        wandb.log(metrics, step=step)
    else:
        # Append metrics to a local log file
