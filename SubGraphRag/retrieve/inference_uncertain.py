...
def main(args):
    ...
    model.eval()  # This will keep dropout active for MC sampling
    ...
    for i in tqdm(range(len(infer_set))):
        ...
        if len(h_id_tensor) != 0:
            ...
            pred_triple_stds = torch.sigmoid(std_logits)
            ...
            top_K_uncertainties = std_logits[top_K_triple_IDs].cpu().tolist()
    ...
    torch.save(pred_dict, os.path.join(root_path, f'retrieval_result_mc{args.mc_samples}.pth'))
    ...
    torch.save(uncertainty_summary, os.path.join(root_path, f'uncertainty_summary_mc{args.mc_samples}.pth'))
