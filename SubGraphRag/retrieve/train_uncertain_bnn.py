...
def elbo_loss(pred_logits, target_probs, model, kl_weight=1.0):
    """
    Compute the Evidence Lower BOund (ELBO) loss.
    ELBO = reconstruction_loss + kl_weight * KL_divergence

    The KL divergence is scaled by the number of data points (batch size),
    which helps balance the influence of the prior relative to the data likelihood.

    Args:
        pred_logits: Model predictions
        target_probs: Target probabilities
        model: BNN model instance with get_kl_divergence() method
...
def train_epoch(device, train_loader, model, optimizer, kl_weight=1.0):
    ...
    for sample in tqdm(train_loader):
        ...
        # Calculate ELBO loss
        loss, nll_loss, kl_loss = elbo_loss(
            pred_triple_logits,
            target_triple_probs,
            model,
            kl_weight
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mc_samples', type=int, default=15,
                       help='Number of Monte Carlo samples for uncertainty estimation (default: 15)')
    parser.add_argument('--num_epochs', type=int, default=None,
                       help='Number of training epochs (overrides config)')
    parser.add_argument('--kl_weight_start', type=float, default=0.00001,
                       help='Initial KL divergence weight for ELBO loss (default: 0.00001)')
    parser.add_argument('--kl_weight_end', type=float, default=0.001,
                       help='Final KL divergence weight for ELBO loss (default: 0.001)')
